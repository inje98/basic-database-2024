...
class qtApp(QMainWindow):
    ...
    def btnSaveClicked(self): # 저장버튼 클릭
        ...
        if mode == 'I': #INSERT 경우
            ...
            cursor = conn.cursor(as_dict = False) # COUNT(*)는 데이터가 딱 1개이기 때문에 as_dict=False
            ...
            # print(cursor.fetchone()[0]) # COUNT(*)은 데이터가 딱 1개이기 때문에 cursor.fetchone() 함수로 (1, ) 튜플을 가져옴
    ...
    def btnReloadClicked(self): # 조회버튼 클릭
        ...
        for row in cursor:
            ...
            lsResult.append(temp)

        conn.close() # DB는 접속해서 일이 끝나면 무조건 닫는다.

        # print(lsResult) # tblBooks 테이블위젯에 표시
        self.makeTable(lsResult)
