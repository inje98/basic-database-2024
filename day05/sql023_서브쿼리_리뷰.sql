-- 서브쿼리 리뷰
-- ALL, ANY(SOME) 쓰는 조건은 아래의 데이터는 사용이 어렵다
...
SELECT SUM(o.saleprice) AS '조인도 가능'
 ...
 -- SELECT절 서브쿼리, JOIN으로 변경가능. 이미 쿼리가 너무 복잡해서 테이블을 추가하기 힘들면,
 -- 많이 사용
...
-- 고객별 판매액 집계 쿼리가 FROM절에 들어가면 모든 속성(컬럼)에 지정되어야 함
SELECT b.total
     , c.name
