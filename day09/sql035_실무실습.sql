...
SELECT FIRST_NAME + ' ' + LAST_NAME AS 'name'
 ...
 -- 단일행 함수, 변환 함수
 -- 샘플문제 : 60번 IT부서에서 사원급여를 12.3% 인상하기로 함. 정수만 반올림하여 보고서
 -- 작성, 사번, 이름, 급여, 인상된 급여(Increased Salary)(5개행)
 SELECT *
   FROM departments;
...
-- 서브쿼리
/* 사원의 급여 정보중 최소 급여를 받는  
   사원의 이름을, 업무, 급여, 입사일 출력(21) */
SELECT *
  FROM employees AS e
...
-- CASE 연산자(프로그래밍적인)
/* 107명의 직원중 HR_REP(10%), MK_REP(12%), PR_REP(15%), SA_RAP(18%), IT_PROG(20%) */
SELECT EMPLOYEE_ID
     , FIRST_NAME + ' ' + LAST_NAME AS 'name'
     , JOB_ID
     , SALARY
